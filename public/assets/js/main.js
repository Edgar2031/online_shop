...
(function($) {
    ...
    if ( document.querySelector('[data-scrollbar="scroll"]') ) {
        ...
        $( '[data-scrollbar="scroll"]' ).each(function() {
          ...
          $(this).niceScroll({
            ...
            cursorborderradius: nicescroll. cursorborderradius,
  ...
  $('[data-collapsing="partially"]').on('click', function () {
    ...
    if($(p_collapsing_item).is('.p-collapse')) {
      ...
      $(p_collapsing_item).removeClass('p-collapse-fixed');
    } else {
      $(p_collapsing_item).addClass('p-collapse');
      $(p_collapsing_item).addClass('p-collapse-fixed');
    }
  });
  // resize
  window.onresize = function() {
    var userScreenSize = window.innerWidth;
    if(!$('#adminNav').is('.p-collapse-fixed')) {
      if(userScreenSize <= breakpoints.xl) {
        $('#adminNav').addClass('p-collapse');
      } else {
        $('#adminNav').removeClass('p-collapse');
      }
    }
  }
  ...
  function sVH() {
    ...
    Array.prototype.forEach.call(sVHItem, function(sVHItemEl) {
      if($(window).width() >= 1200) {
        ...
        sVHFooter = document.querySelector('footer').clientHeight,
        sVHBody = clientHeight - scrollLeft.top - sVHFooter - 1 - 16;
  ...
  function sVHF() {
    ...
    Array.prototype.forEach.call(sVHItem, function(sVHFItemEl) {
      if($(window).width() >= 992) {
        ...
        sVHFFooter = document.querySelector('footer').clientHeight,
        sVHF_form_control = document.getElementById('sVHF_form').clientHeight,
        sVHF_space_Y = 16,
        sVHFBody = clientHeight - scrollLeft.top - sVHFFooter - sVHF_form_control - sVHF_space_Y - 1 - 16;
  ...
  // collapse (placeholder demo)
  let collapsePanel_btn = document.querySelectorAll('.panel-header-nav [data-toggle="collapse"]');
  Array.prototype.forEach.call(collapsePanel_btn, function(cleBtn) {
    cleBtn.addEventListener('click', function(e) {
      let cle_btn_target = this.closest('.panel-card');
      if($(cle_btn_target).is('.clsd_c')) {
        $('.panel-card').animate({
          height: "100%"
        }, 350 );
        cle_btn_target.classList.remove('clsd_c');
      } else {
        cle_btn_target.style.height = 'auto';
        cle_btn_target.classList.add('clsd_c');
      }
    });
  });
  ...
  // dropdown-clickable
  $('.dropdown-clickable').click(function(e) {
    e.stopPropagation();
  });
